...
/*
========================================
 Chức năng: Xử lý các hành động quản trị điểm số (CRUD, xóa nhiều, thêm, sửa, xóa)
========================================
*/
...
function handleCreateGrade() {
    ...
    if (!isset($_POST['student_id']) || !isset($_POST['subject_id']) || !isset($_POST['attendance_score']) || !isset($_POST['midterm_score']) || !isset($_POST['final_score']) || !isset($_POST['total_score'])) {
    ...
    $attendance_score = trim($_POST['attendance_score']);
    $midterm_score = trim($_POST['midterm_score']);
    $final_score = trim($_POST['final_score']);
    $total_score = trim($_POST['total_score']);

    // Kiểm tra dữ liệu không được để trống
    if ($student_id === '' || $subject_id === '' || $attendance_score === '' || $midterm_score === '' || $final_score === '' || $total_score === '') {
        header("Location: ../views/grade_create.php?error=Vui lòng điền đầy đủ thông tin");
        exit();
    }

    // Kiểm tra điểm hợp lệ (0-10)
    foreach (['attendance_score' => $attendance_score, 'midterm_score' => $midterm_score, 'final_score' => $final_score, 'total_score' => $total_score] as $label => $score) {
        if (!is_numeric($score) || $score < 0 || $score > 10) {
            header("Location: ../views/grade_create.php?error=$label phải là số từ 0 đến 10");
            exit();
        }
    }
    ...
    // Thực hiện thêm điểm
    $success = addGrade($student_id, $subject_id, $attendance_score, $midterm_score, $final_score, $total_score);
    if ($success) {
...
function handleEditGrade() {
    ...
    // Kiểm tra đầy đủ thông tin đầu vào
    if (!isset($_POST['id']) || !isset($_POST['student_id']) || !isset($_POST['subject_id']) || !isset($_POST['attendance_score']) || !isset($_POST['midterm_score']) || !isset($_POST['final_score']) || !isset($_POST['total_score'])) {
    ...
    foreach (['attendance_score' => $attendance_score, 'midterm_score' => $midterm_score, 'final_score' => $final_score, 'total_score' => $total_score] as $label => $score) {
        if (!is_numeric($score) || $score < 0 || $score > 10) {
            header("Location: ../views/grade_edit.php?id=$id&error=$label phải là số từ 0 đến 10");
            exit();
        }
    }
    ...
    $success = updateGrade($id, $student_id, $subject_id, $attendance_score, $midterm_score, $final_score, $total_score);
    if ($success) {
...
/**
 * Xử lý xóa nhiều điểm (bulk)
 */
function handleBulkDeleteGrade() {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        header("Location: ../views/grade.php?error=Phương thức không hợp lệ");
        exit();
    }
    ...
    if (!is_array($ids) || empty($ids)) {
        header("Location: ../views/grade.php?error=Không có mục nào được chọn để xóa");
        exit();
    }
    ...
    foreach ($ids as $id) {
        ...
        if (deleteGrade($id)) $success++; else $failed++;
    }

    $msgs = [];
    if ($success > 0) $msgs[] = "Đã xóa $success mục.";
    if ($failed > 0) $msgs[] = "Không thể xóa $failed mục.";
    $msg = implode(' ', $msgs);
    header('Location: ../views/grade.php' . ($msg ? '?success=' . urlencode($msg) : ''));
    exit();
}
