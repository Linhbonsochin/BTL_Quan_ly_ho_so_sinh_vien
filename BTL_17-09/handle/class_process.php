...
function handleBulkDelete() {
    ...
    // Kiểm tra xem có sinh viên liên kết với các lớp này không
    $conn = getDbConnection();
    $idList = implode(',', $ids);
    $sqlChk = "SELECT class_id, COUNT(*) as cnt FROM students WHERE class_id IN ($idList) GROUP BY class_id";
    ...
    if ($result) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $hasStudents = true;
            $classesWithStudents[] = $row['class_id'];
        }
        
        if ($hasStudents) {
            header("Location: ../views/class.php?error=Không thể xóa lớp có sinh viên liên kết");
            exit();
        }
    }
    ...
    if ($success) {
        header("Location: ../views/class.php?success=Xóa các lớp đã chọn thành công");
    } else {
