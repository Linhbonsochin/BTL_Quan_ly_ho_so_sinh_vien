...
/*
========================================
 Chức năng: Xác thực người dùng, kiểm tra đăng nhập, đăng xuất, lấy thông tin user
========================================
*/
/**
...
function authenticateUser($conn, $username, $password)
{
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        // Prefer secure hashed passwords
        if (!empty($user['password']) && password_verify($password, $user['password'])) {
            mysqli_stmt_close($stmt);
            return $user;
        }

        // Fallback: if legacy plaintext password is stored, accept it and upgrade to a hashed password
        if ($password === $user['password']) {
            // Attempt to upgrade the stored password to a secure hash
            ...
            if ($upd) {
                ...
                // reflect new hash in returned user array
