...
function getAllSubjects()
{
    ...
    $sql = "SELECT s.id, s.subject_code, s.subject_name, s.credits, s.department_id, d.department_name " .
        "FROM subjects s LEFT JOIN departments d ON s.department_id = d.id ORDER BY s.subject_name";
    $items = [];
    try {
...
function getSubjectById($id)
{
    ...
    $sql = "SELECT s.id, s.subject_code, s.subject_name, s.credits, s.department_id, d.department_name " .
        "FROM subjects s LEFT JOIN departments d ON s.department_id = d.id WHERE s.id = ? LIMIT 1";
    try {
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
