...
require_once __DIR__ . '/../functions/subject_functions.php'; // File chứa các hàm xử lý môn học
...
// consider switching to DB LIMIT/OFFSET in functions/subject_functions.php
$perPage = 10;
...
$pageSubjects = array_slice($subjects, $start, $perPage);
?>
<!doctype html>
<html lang="vi">
    ...
    <div class="d-flex">
        ...
        <div class="flex-grow-1 dashboard-main">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h3 class="m-0">Quản lý môn học</h3>
                <div>
            ...
            <div class="mb-3">
                <a href="subject/subject_create.php" class="btn btn-primary">Thêm môn</a>
            </div>

            <?php if (isset($_GET['success'])): ?>
                <div class="alert alert-success"><?php echo htmlspecialchars($_GET['success']); ?></div><?php endif; ?>
            ...
            <div class="card card-ghost p-3">
                ...
                <form id="bulkForm" method="POST" action="/BTL_17-09/handle/subject_process.php">
                        ...
                        <table class="table table-bordered" style="text-align: center;">
                            ...
                            <tbody>
                                <?php if (empty($subjects)): ?>
                                    <tr>
                                        <td colspan="7" class="text-center p-4">Chưa có môn học</td>
                                    </tr>
                                <?php else:
                                    foreach ($pageSubjects as $i => $s): ?>
                                        <tr>
                                            <td class="bulk-col d-none" style="vertical-align:middle;"><input type="checkbox"
                                                    name="ids[]" value="<?php echo $s['id']; ?>" class="bulk-checkbox"></td>
                                            ...
                                            <td><?php echo isset($s['major_name']) ? htmlspecialchars($s['major_name']) : '-'; ?></td>
                                            <td class="table-actions">
                                                <a href="subject/subject_edit.php?id=<?php echo $s['id']; ?>" class="text-primary"
                                                    title="Sửa"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                                                <a href="/BTL_17-09/handle/subject_process.php?action=delete&id=<?php echo $s['id']; ?>"
    ...
    <script>
        (function () {
            ...
            function updateBulkCount() {
                ...
                var conf = document.getElementById('confirmCount'); 
                if (conf) conf.textContent = c;
                var info = document.getElementById('bulkInfo'); 
                if (c > 0) info.style.display = 'block'; 
                else if (!bulkMode) info.style.display = 'none';
            }
            ...
            document.getElementById('toggleBulk').addEventListener('click', function (e) { 
                ...
                // if turning off and items are selected, show confirmation
                if (bulkMode) {
                    var cSel = document.querySelectorAll('.bulk-checkbox:checked').length;
                    if (cSel > 0) {
